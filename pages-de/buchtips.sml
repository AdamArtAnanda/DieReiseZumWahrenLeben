Page {
	title: "Buchtipps"
	padding: "8"
	scrollable: "true"
	
	Column {
		padding: "8"



		Spacer {amount: 16}
		Markdown { text: "# Buchtips"}
		Row {
			padding: "8 0 0 0"
			
			Markdown {text:"**Buchtitel**" weight:1}
			Markdown {text:"**Autor**" weight:1}
		}
		Spacer {amount: 8}
		Row {
			padding: "8 0 0 0"
			Markdown {text:"Entfalte dein erotisches Potential" weight:1}
			Markdown {text:"Sheri Winston" weight:1}
		}
		Spacer {amount: 8}
		Row {
			padding: "8 0 0 0"
			Markdown {text:" Schule für Götter" weight:1}
			Markdown {text:"Stefano D‘Anna" weight:1}
		}
		Spacer {amount: 8}
		Row {
			padding: "8 0 0 0"
			Markdown {text:"The Multi-Orgasmic-Man" weight:1}
			Markdown {text:"Mantak Chia" weight:1}
		}
		Spacer {amount: 8}
		Row {
			padding: "8 0 0 0"
			Markdown {text:"I Ging. Das Buch der Wandlungen" weight:1}
			Markdown {text:"Richard Willhelms" weight:1}
		}
		Spacer {amount: 8}
		Row {
			padding: "8 0 0 0"
			Markdown {text:"Die Macht ihres Unterbewusstseins" weight:1}
			Markdown {text:"Joseph Murphy" weight:1}
		}
		Spacer {amount: 8}
		Row {
			padding: "8 0 0 0"
			Markdown {text:"Das UBUNTU Prinzip: Ein revolutionärer Plan für gerechten Wohlstand" weight:1}
			Markdown {text:"Michael Tellinger" weight:1}
		}
		Spacer {amount: 8}
		Row {
			padding: "8 0 0 0"
			Markdown {text:"Das Kybalion, die 7 hermetischen Gesetze" weight:1}
			Markdown {text:"Anonym von die drei Eingeweihten" weight:1}
		}
		Spacer {amount: 8}
		Row {
			padding: "8 0 0 0"
			Markdown {text:"Verändere deine Glaubenssätze mit der Kraft deiner intelligenten Zellen" weight:1}
			Markdown {text:"Christoph Simo" weight:1}
		}
		Spacer {amount: 8}
		Row {
			padding: "8 0 0 0"
			Markdown {text:"Erotische Intelligenz" weight:1}
			Markdown {text:"Jack Morin" weight:1}
		}
		Spacer {amount: 8}
		Row {
			padding: "8 0 0 0"
			Markdown {text:"[Unterhaltung mit meinem höheren Selbst](https://books.crowdware.at/index_de.html)" weight:1}
			Markdown {text:"Adam Art Ananda" weight:1}
		}
		Spacer {amount: 8}
		Row {
			padding: "8 0 0 0"
			Markdown {text:"[Camp Eden -  Wir wir unser Paradies wiedererschafft haben](https://books.crowdware.at/index_de.html)" weight:1}
			Markdown {text:"Adam Art Ananda" weight:1}
		}
		Spacer {amount: 8}
		Row {
			padding: "8 0 0 0"
			Markdown {text:"[Step Out - A guideline how to step out of the system](https://books.crowdware.at/index_de.html)" weight:1}
			Markdown {text:"Adam Art Ananda" weight:1}
		}
		Spacer {amount: 8}
		Row {
			padding: "8 0 0 0"
			Markdown {text:"[Manifestiere ein besseres Leben - Ich mach mir die Welt, widde widde wie sie mir gefällt ](https://books.crowdware.at/index_de.html)" weight:1}
			Markdown {text:"Adam Art Ananda" weight:1}
		}
		Spacer {amount: 8}
		Row {
			padding: "8 0 0 0"
			Markdown {text:"[Meine Heldenreise - Eine Reise zu mir selbst](https://books.crowdware.at/index_de.html)" weight:1}
			Markdown {text:"Adam Art Ananda" weight:1}
		}
		Spacer {amount: 8}
		Row {
			padding: "8 0 0 0"
			Markdown {text:"[Post Corona Gesellschaft - Wie wir das Land wieder aufgebaut haben, nach dem es unsere Regierung beinahe gegen die Wand gefahren hat](https://books.crowdware.at/index_de.html)" weight:1}
			Markdown {text:"Adam Art Ananda" weight:1}
		}

	
		Spacer {amount: 8}
		Row {
			Button {label: "< Glossar" link: "page:glossar" weight: 1}
			Spacer {amount: 8}
			Button {label: "<< Home" link: "page:home" weight: 1}
		}
	}
}